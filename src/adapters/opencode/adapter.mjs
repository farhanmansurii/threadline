...
export async function installCommands(commands, { replace = false } = {}) {
  ...
  for (const command of commands) {
    const content = await readTemplate(`templates/opencode/command-${command}.md`).catch(() => null);
    if (!content) continue;
...
export async function installConfig({ replace = false } = {}) {
  ...
  const managed = `# Threadline-managed config for OpenCode\n# OpenCode rules and runtime config are generated by Threadline.\n`;
  if (replace) {
...
export async function adopt() {
  const findings = [];
  findings.push(await inspectPath('opencode-skill', path.join(homeDir, 'skills', 'threadline', 'SKILL.md')));
  findings.push(await inspectPath('opencode-config', path.join(homeDir, 'config.toml')));
  return findings;
}
